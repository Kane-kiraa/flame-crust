...
export default function StandaloneKitchenDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Top Navigation Bar */}
      <header className="h-[calc(env(safe-area-inset-top)+4rem)] pt-[env(safe-area-inset-top)] border-b border-border/50 bg-card/50 backdrop-blur-md flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center shadow-md shadow-orange-500/20">
            <ChefHat className="size-5 text-white" />
          </div>
          <div>
            <h1 className="font-serif text-lg font-bold text-foreground">Kitchen Display</h1>
            <p className="text-xs text-muted-foreground font-medium">Flame & Crust</p>
          </div>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex flex-col items-end mr-2">
            <span className="text-sm font-bold text-foreground">{user.name || "Chef"}</span>
            <span className="text-[10px] text-muted-foreground uppercase font-bold">{user.role_title || "Staff"}</span>
          </div>
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/50 border border-border/50">
            <div className={cn("size-2 rounded-full animate-pulse", user.status === "ONLINE" ? "bg-green-500" : user.status === "BUSY" ? "bg-yellow-500" : "bg-gray-500")} />
            <span className="text-xs font-semibold text-foreground">{user.status || "ONLINE"}</span>
          </div>
      ...
      {/* Main Board Area */}
      <main className="flex-1 p-6 overflow-hidden flex flex-col h-[calc(100vh-4rem-env(safe-area-inset-top,0px))]">
        {loading ? (
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 overflow-hidden pb-2">
            ...
            <div className="flex flex-col bg-secondary/20 rounded-3xl border border-border/50 overflow-hidden shadow-sm">
            ...
            {/* Preparing Column */}
            <div className="flex flex-col bg-orange-500/5 rounded-3xl border border-orange-500/20 overflow-hidden shadow-sm">
            ...
            {/* Ready Column */}
            <div className="flex flex-col bg-green-500/5 rounded-3xl border border-green-500/20 overflow-hidden shadow-sm">
